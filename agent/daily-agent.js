...
const MemeConcept = z.object({
  found_existing: z
    .boolean()
    .describe("true if a real existing meme was found on the web"),
  source_url: z
    .string()
    .nullable()
    .describe("URL of the found meme, null if invented"),
  animal: z.string().describe("the trash animal featured. NEVER a rat."),
  caption: z
    .string()
    .describe("the (de)motivational text, lowercase, sincere-but-unhinged"),
  mood: z.enum(MOODS),
  image_description: z
    .string()
    .describe("scene description for image generation if invented"),
});
...
export async function runDailyBlessing() {
  ...
  const used = [...entriesOf(seed), ...blessings.raccoons].map(
    (r) => r.caption,
  );
  ...
  const creative = {
    mood: pick(MOODS),
    animals: pickN(
      ALLOWED_ANIMALS.filter((a) => !recentAnimals.includes(a)),
      3,
    ),
  };
